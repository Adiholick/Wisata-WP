...
if($action == "tambah-tempat"){
    ...
    $transportasi = mysqli_real_escape_string($con, $_POST['transportasi']);
    $tiket = mysqli_real_escape_string($con, $_POST['tiket']);
    $fasilitas = mysqli_real_escape_string($con, $_POST['fasilitas']);
    $kebersihan = mysqli_real_escape_string($con, $_POST['kebersihan']);
    $keindahan = mysqli_real_escape_string($con, $_POST['keindahan']);
    $deskripsi = mysqli_real_escape_string($con, $_POST['deskripsi']);

    $sql_tambah = mysqli_query($con, "INSERT INTO `tempat`(`id`, `nama`, `provinsi_id`, `transportasi`, `tiket`, `fasilitas`, `kebersihan`, `keindahan`, `deskripsi`) VALUES (NULL ,'$nama_tempat', '$provinsi', '$transportasi','$tiket','$fasilitas','$kebersihan','$keindahan','$deskripsi')");
    if($sql_tambah){
    ...
    if($error_file == UPLOAD_ERR_OK){
        if(in_array($ext_file, $ext_validate)){
            ...
            $insert_foto = mysqli_query($con, "INSERT INTO `foto`(`id_tempat`, `file`) VALUES ('$id','$nama_foto')");
            if($insert_foto){
                $return = array(
                    "error" => false,
                    "pesan"=> "sukses"
    ...
    $id = mysqli_real_escape_string($con, $_POST['id-tempat2']);
    $query_load_table = mysqli_query($con, "SELECT * FROM `foto` WHERE `id_tempat`='$id'");
    ...
    $sql_tambah = mysqli_query($con, "UPDATE `tempat` SET `nama`='$nama_tempat', `provinsi_id`='$provinsi' ,`transportasi`='$transportasi',`tiket`='$tiket',`fasilitas`='$fasilitas',`kebersihan`='$kebersihan',`keindahan`='$keindahan',`deskripsi`='$deskripsi' WHERE `id`='$id'");
    if($sql_tambah){
        $last_id = mysqli_insert_id($con);
        $return = array(
            "error" => false,
    ...
    $sql_foto = mysqli_query($con, "SELECT * FROM `foto` WHERE `id_tempat`='$id'");
